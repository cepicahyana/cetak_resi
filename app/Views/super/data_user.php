			...
			<div class="row">
				<div class="col-md-12">
					<div class="card">
						...
						<div class="row">
							<div class="col-md-3">
							<label class="black control-label">Group user</label>	
							<select class="form-control show-tick" id="f1" data-live-search="true" onclick="reload_table()">
							...
							<div id="area_lod">
								...
								<table id='table' class="tabel black table-bordered table-hover table-striped dataTable" style="font-size:12px;width:100%">
									<thead  class='sadow bg-teal'>			
										...
										<th class='thead' style="min-width:80px">PROCESS</th>
									</thead>
								</table>
								</div>
							</div>
...
<script type="text/javascript">
	var  dataTable = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?= site_url('super/data_tables_user');?>",
            "type": "POST",
			"data": function ( data ) {
				data.f1 = $('#f1').val();
				//data.f2 = $('#f2').val();
...
{ 
	...
	$("#formSubmit_input").attr("url","<?= base_url("super/insert_User");?>");
	$.post("<?= site_url("super/input_User"); ?>",function(data){
		$("#add_page").html(data);
		$("#formSubmit_input")[0].reset();
		$("#inputPreviewImg").attr("src", '<?= base_url()?>/public/theme/images/user/img_not.png');
	});
...
{	
	...
	$("#formSubmit_edit").attr("url","<?= base_url("super/update_User");?>");
	$.post("<?= site_url("super/edit_User"); ?>",{id:id},function(data){
		$("#edit_page").html(data);
	});
}
...
function del(id,level,name){
   alertify.confirm("Delete","<center>Delete data <b>Name : "+name+"</b>?</center>",function(){
		$.ajax({
			 url:'<?= site_url("super/delete_User"); ?>',
